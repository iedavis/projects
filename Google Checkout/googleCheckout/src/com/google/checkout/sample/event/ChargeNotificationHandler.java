...
/**
 * The <b>ChargeNotificationHandler</b> defines the process for handling 
 * a charge amount notification from Google Checkout. The constructor calls 
 ...
 * for ChargeAmountNotification events.
 * <br>
 * When a charge amount notification is received, the 
 * &lt;charge-amount-notification&gt; request will be routed from the 
 * {@see CallBackServlet} to the {@see CallBackParser}. The 
 * {@see CallBackParser} will then call the <b>handleEvent</b> method of 
 * each class that implements the {@see ChargeNotificationListener}.
 * <br>
 ...
 * send the charge amount information to the appropriate internal 
 * systems so that those systems can update the order status and 
 * appropriately respond to the charge.
 *
 * @version 1.0 beta
 */
public class ChargeNotificationHandler extends CallBackHandler
    implements ChargeNotificationListener {
  
  public ChargeNotificationHandler() throws ProtocolException {
    super();
  ...
  public void handleEvent(CallBackEvent event) {
    /*
     * +++ CHANGE ME +++
     * Implement this method to handle charge amount notifications.
     * To retrieve the JAXB objects that were unmarshalled from the
     * charge amount notification, you can use the following code:
     * <code>ChargeAmountNotification notification = 
     *     event.getChargeAmountNote();</code>
     * You can then access individual components of the notification 
     * by calling the methods in the generated JAXB classes.
     * For example:<br>
     * <code>Money latestChargeAmount = 
     *     notification.getLatestChargeAmount();</code><br>
